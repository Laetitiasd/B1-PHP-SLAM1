<?php
	...
	function calculerPrixPromo($produit){
		...
		if ($promo > 0) {
			$prix_promo = $prixUnit - ($prixUnit * ($promo / 100));
				return $prix_promo;
		} else {
				return $prixUnit;
    }
...
HTML;
    ...
    if ($nomFichier != "") {
        $nomFichier = "vue-produit-$code.html";
        $dest = fopen($nomFichier, "w");
        fwrite($dest, $html);
        fclose($dest);
    }
	...
	function mainTest(){
		...
		echo "\n6.a) -------------------------------------\n" ;
			$codeHTML1 = genererListeHTML($produitTest1, "produitFraise") ;
			echo "Code HTML généré pour le produit 1 :\n$codeHTML1\n" ;

		echo "\n6.b) -------------------------------------\n" ;
			$codeHTML2 = genererListeHTML($produitTest2, "produitSelMarin") ;
			echo "Code HTML généré pour le produit 2 :\n$codeHTML2\n" ;

		
		// Exercice 7
		echo "\n7.a) -------------------------------------\n" ;
		// Votre code ici
			$produitTest3 = "872:Shampooing:5:20"; 
			echo "Produit de test 3 : $produitTest3\n";
		
		echo "\n7.b) -------------------------------------\n" ;
		// Votre code ici
			$codeHTML3 = genererTableHTML($produitTest3, "vue-produit-872");
			echo "Code HTML généré pour le produit 3 :\n$codeHTML3\n";
	}
