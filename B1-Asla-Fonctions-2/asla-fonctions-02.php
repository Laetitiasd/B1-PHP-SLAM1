<?php
	...
	function calculerMoyennePromos(){
		...
		foreach( $produits as $unProduit ){
			...
			if( estEnPromo( $unProduit ) == TRUE ){
				...
				$totalPromos += getPromotion($unProduit);
				$nbPromos++;
	...
	function appliquerMemePromoTousProduits( $promo ){
        ...
        list($code, $nom, $prixUnit, $anciennePromo) = explode(":", $produit);
			$produit = "$code:$nom:$prixUnit:$promo";
	...
	function appliquerPromoProduit( $code , $promo ){
		...
		if( $ind != -1 ){
			// Votre code ici
			 list($prodCode, $nom, $prixUnit, $anciennePromo) = explode(":", $produits[$ind]);
			 $produits[$ind] = "$prodCode:$nom:$prixUnit:$promo";
		}
	}
